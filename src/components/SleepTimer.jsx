    ...
    <div className="card">
      ...
      {timerActive&&timeLeft!==null&&timeLeft<=60&&timeLeft>0 ? (
        <button onClick={bumpTimer} className="btn-primary"
          style={{background:bm?'#2a2114':'var(--c-accent-strong)',color:bm?'#e6dcce':'var(--c-bg)',animation:'pulse 1.5s ease-in-out infinite',
            boxShadow:bm?'none':'0 0 20px rgba(79,70,229,.4)'}}>
          Still Awake? (+15 min)
        </button>
      ):(
        <button onClick={toggleTimer} className="btn-primary"
          style={{background:timerActive?(bm?'rgba(255,255,255,.04)':'rgba(248,113,113,.1)'):(bm?'#2a2114':'var(--c-accent-strong)'),
            color:timerActive?(bm?'#8a7860':'#f87171'):(bm?'#e6dcce':'var(--c-bg)'),
