...
export default function PodcastScreen({ show, onClose }) {
  ...
  const pill = (active=false) => ({ padding:'.55rem .9rem',borderRadius:'.7rem',border:`1px solid ${c_bord}`,fontSize:'.78rem',fontWeight:700,cursor:'pointer',background:active?'#b8813a':'rgba(230,178,119,.14)',color:active?'#fff':'#e6b277' });
              ...
              <div className="card" style={{padding:'.875rem',marginBottom:'1rem'}}>
                <div style={{display:'flex',gap:'.5rem'}}>
                  ...
                  <button onClick={()=>{ if(rssUrl.trim()){ loadFeed(); setAdding(false); setFeed(null); setView('podcast'); } }} disabled={!rssUrl.trim()||loading}
                    style={{padding:'.5rem 1rem',borderRadius:'.625rem',background:rssUrl.trim()?'#b8813a':'#4a3a1e',color:'#fff',border:'none',fontWeight:700,cursor:rssUrl.trim()?'pointer':'default',whiteSpace:'nowrap'}}>
                    {loading?'…':'Load'}
                  </button>
                </div>
