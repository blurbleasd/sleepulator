...
2. Focus R5: phase-end local notifications when backgrounded.
3. Idle-based scene freeze without a running timer.
4. Scene A/B on device: retuned Metal aurora (star twinkle decorrelated, motion up, lateral travel) and the v2 rain-depth lens shader — both tuned blind, expect a knob-turning round.
5. Wake alarm (gentle rising audio — reverse of the sleep fade) and a StandBy-mode check of the Live Activity: discussed 2026-07-04, need design input / a device.
6. ~~Focus resume-label cross-mode copy bug (FOCUS-MODE-SPEC R2)~~ — fixed 2026-07-04 (HomeView statusText mirrors the palette snap).
